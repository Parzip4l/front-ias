        ...
        <div class="card shadow-sm border-0 rounded-4 overflow-hidden">
            ...
            <div class="bg-primary p-4 text-white d-flex align-items-center">
                ...
                <div class="ms-auto d-flex">
                    <a href="{{ route('flow.index') }}" class="btn btn-light btn-sm me-2">
                        <i class="bi bi-arrow-left"></i> Kembali
                    </a>
                    <a href="{{ route('flow.edit', $flow['id']) }}" class="btn btn-warning btn-sm">
            ...
            <div class="card-body p-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <ul class="list-unstyled mb-0">
                            ...
                            <li class="mb-3">
                                ...
                                <span class="fw-semibold">{{ $flow['approval_type'] ?? '-' }}</span>
                            </li>
                            <li class="mb-3">
                ...
                <div class="mt-5">
                    @if($flow['approval_type'] === 'hirarki')
                        ...
                        @endif
                    @elseif($flow['approval_type'] === 'nominal')
                    <!-- Nominal Flow -->
                    <div class="d-flex align-items-center mb-3">
                    ...
                    @if(!empty($amountFlows))
                    <div class="accordion" id="amountFlowAccordion">
                        @foreach($amountFlows as $index => $flowAmount)
                            <div class="accordion-item mb-3 shadow-sm rounded">
                                <h2 class="accordion-header" id="heading{{ $index }}">
                                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse{{ $index }}">
                                        Rp {{ number_format($flowAmount['min_amount'],0,',','.') }} - 
                                        {{ $flowAmount['max_amount'] ? number_format($flowAmount['max_amount'],0,',','.') : 'Unlimited' }}
                                    </button>
                                </h2>
                                <div id="collapse{{ $index }}" class="accordion-collapse collapse" data-bs-parent="#amountFlowAccordion">
                                    <div class="accordion-body p-3">
                                        <div class="d-flex justify-content-between align-items-center mb-2">
                                            <h6 class="mb-0">Steps</h6>
                                            <div>
                                                <button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#addStepAmountModal" data-flow-id="{{ $flowAmount['id'] }}">
                                                    <i class="bi bi-plus-lg"></i> Tambah Step
                                                </button>
                                                <button class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#editAmountFlowModal" data-flow-id="{{ $flowAmount['id'] }}">
                                                    <i class="bi bi-pencil"></i> Edit
                                                </button>
                                                <button class="btn btn-sm btn-danger btn-delete-flow" data-id="{{ $flowAmount['id'] }}">
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-hover align-middle mb-0">
                                                ...
                                                <tbody>
                                                    @forelse($flowAmount['steps'] as $step)
                                                    <tr>
                                                        <td><span class="badge bg-info">{{ $step['step_order'] ?? '-' }}</span></td>
                                                        <td>{{ $step['user']['name'] ?? '-' }}</td>
                                                        <td>{{ $step['division']['name'] ?? '-' }}</td>
                                                        ...
                                                        <td>
                                                            ...
                                                            <button class="btn btn-sm btn-danger btn-delete-step" data-id="{{ $step['id'] }}">
                                                                <i class="bi bi-trash"></i> Hapus
                                                            </button>
                                                        </td>
                                                    </tr>
...
<div class="modal fade" id="addAmountFlowModal">
    <div class="modal-dialog">
        <form method="POST" action="{{ route('amountflow.store') }}" class="modal-content">
            @csrf
...
<div class="modal fade" id="addStepAmountModal">
    <div class="modal-dialog">
        <form method="POST" action="{{route('amountstep.store')}}" class="modal-content">
            @csrf
            <div class="modal-header">
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    document.querySelectorAll("[data-bs-target='#editAmountFlowModal']").forEach(button => {
        button.addEventListener("click", function() {
            ...
            const flowData = {!! json_encode($amountFlows) !!}.find(f => f.id == flowId);
