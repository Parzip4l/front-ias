...
<head>
    ...
    <title>Ringkasan SPPD {{ $sppd['nomor_sppd'] ?? '' }}</title>
    <style>
        body {
            margin: 0;
    ...
    <div class="card" id="public-sppd-card">
        <div class="header">
            <div>
                ...
                <h1>Ringkasan SPPD: {{ $sppd['nomor_sppd'] ?? '-' }}</h1>
            </div>
            <span class="badge">{{ $sppd['status'] ?? 'DRAFT' }}</span>
        </div>
        ...
        <div class="body">
            ...
            <div class="grid">
                <div>
                    ...
                    <div class="value">{{ $sppd['user']['name'] ?? '-' }}</div>
                </div>
                <div>
                    <div class="label">Nomor SPPD</div>
                    <div class="value">{{ $sppd['nomor_sppd'] ?? '-' }}</div>
                </div>
                <div>
                    <div class="label">NIK</div>
                    <div class="value">{{ $sppd['user']['employee']['employee_number'] ?? '-' }}</div>
                </div>
                <div>
                    <div class="label">Divisi & Jabatan</div>
                    <div class="value">{{ $sppd['user']['employee']['division']['name'] ?? '-' }} - {{ $sppd['user']['employee']['position']['name'] ?? '-' }}</div>
                </div>
                <div>
                    <div class="label">Tujuan Perjalanan</div>
                    <div class="value">{{ $sppd['tujuan'] ?? ($tujuan[0]['province']['name'] ?? '-') }}</div>
                </div>
                <div>
                ...
                <div style="grid-column: 1 / -1;">
                    ...
                    <div class="value">{{ $sppd['keperluan'] ?? '-' }}</div>
                </div>
            </div>
            ...
            <div style="overflow-x:auto;">
                <table>
                    ...
                    <tbody>
                        @forelse($expense as $index => $ex)
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td>{{ $ex['kategori'] ?? '-' }}</td>
                                <td>{{ $ex['deskripsi'] ?? '-' }}</td>
                                <td class="text-end">Rp{{ number_format((float) ($ex['jumlah'] ?? 0), 0, ',', '.') }}</td>
                            </tr>
                        @empty
                            <tr>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const filename = @json($pdfFilename);
